...
def make_stub(name, signature, constants, device_type):
    ...
    if cache_path is None:
        with tempfile.TemporaryDirectory() as tmpdir:
            src = generate_launcher(constants, signature, device_type)
            print("src:")
            print(src)            
...
def generate_launcher(constants, signature, device_type):
...
static void _launch(int gridX, int gridY, int gridZ, int num_warps, int num_ctas, int clusterDimX, int clusterDimY, int clusterDimZ, int shared_memory, CUstream stream, CUfunction function{', ' + arg_decls if len(arg_decls) > 0 else ''}) {{
  void *params[] = {{ {', '.join(f"&arg{i}" for i in signature.keys())} }};
  if (gridX*gridY*gridZ > 0) {{
    if (num_ctas == 1) {{
